...
exports.handleStart = async ({ request, page }, requestQueue, zip, originalUrl, PTCData) => {
    ...
    for (let tid in tables) {
        ...
        results.push({
            ...
            "Cancellation Fee": CancellationFee.match(/^\d+|\d+\b|\d+(?=\w)/g) ? CancellationFee.match(/^\d+|\d+\b|\d+(?=\w)/g)[0] : '',
            "Offer Notes": "",
            "Renewable Blend": RenewableBlend.replace("%", "").replace("No", "").replace("Yes", "").replace("Renewable Energy: ", "").replace(".00", "").trim(),
            ...
            "Rate units": "$/kWh",
            "Termination Notes": CancellationFee.match(/.*\:(.*)/)[1].trim(),
