...
const findArraysUrl = async (urlsCatHtml, country) => {
    ...
    const url = country === COUNTRY.CZ ? 'https://nakup.itesco.cz/groceries/cs-CZ/shop' : 'https://potravinydomov.itesco.sk/groceries/sk-SK/shop';
    arr.map((item) => {
        resultArrUrls.push({
            url: `${url}${item}`,
        });
...
const getProductImage = (productId, reduxResults) => {
    try {
        if (reduxResults[productId]) {
            const { serializedData: { product: { serializedData } } } = reduxResults[productId];
            if (serializedData) {
                return serializedData.defaultImageUrl;
            }
        }
        return null;
    } catch (e) {
...
async function ExtractItems($, country, uniqueItems, stats, request) {
    ...
    if (reduxData) {
        try {
            ...
            const { pages: { serializedData } } = results;
            const [data] = serializedData.filter((item) => item !== null);
            resultsData = data.serializedData;
        } catch (e) {
            log.error(e);
        }
    }
    ...
    if ($('.product-list--list-item')) {
        $('.product-list--list-item').each(function () {
            ...
            if (parseInt($(this).find('.tile-content'))) {
                result.itemId = parseInt($(this).find('.tile-content').attr('data-auto-id'));
            } else if ($(this).find('a.product-image-wrapper')) {
                result.itemId = parseInt($(this).find('a.product-image-wrapper').attr('href').replace(/^.+products\//, ''));
            }
            if ($(this).find('.product-image-wrapper')) {
                result.itemUrl = `${rootUrl}${$(this).find('.product-image-wrapper').attr('href')}`;
            }
            ...
            if (promotionProduct) {
                ...
                if (country === COUNTRY.CZ) {
                    ...
                    result.currentPrice = offer !== '' ? formatPrice(offer.split('nyní')[1]) : formatPrice($(this).find('.price-control-wrapper').text().split(' ')[0]);
                    result.originalPrice = offer !== '' ? formatPrice(offer.replace(/^.+cena|nyní.+/g, '')) : null;
                } else {
                    result.currentPrice = offer !== '' ? formatPrice(offer.split('teraz')[1]) : formatPrice($(this).find('.price-control-wrapper').text().split(' ')[0]);
            ...
            if ($(this).find('.weightedProduct-text').length !== 0) {
                const unitOfMeasure = $(this).find('.weightedProduct-text').text().trim();
...
module.exports = { flat, ExtractItems, findArraysUrl, formatPrice, getProductImage };
